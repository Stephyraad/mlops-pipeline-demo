...
def test_numeric_features_range() -> None:
    ...
    assert(df['Age']).between(15, 65).all()
    assert(df['Years_at_Company']).between(1, 50).all()

def test_ordinal_features_range() -> None:
    df = load_data(DATA_PATH)

    assert(df['Job_Level']).between(1, 5).all()
    assert(df['Work_Life_Balance']).between(1, 5).all()
